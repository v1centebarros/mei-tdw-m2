...
export function ComparisonCard({ card, otherCard }: Readonly<ComparisonCardProps>) {
  const getDifference = (value1: number | string, value2: number | string | undefined) => {
  ...
  const renderDifference = (label: string, value1: number | string, value2: number | string | undefined) => {
  ...
  const comparisonData = [
    ...
    { label: "Toughness", value1: card.toughness, value2: otherCard?.toughness },
    { label: "Loyalty", value1: card.loyalty, value2: otherCard?.loyalty },
    { label: "Colors", value1: card.colors?.length, value2: otherCard?.colors?.length },
    { label: "Types", value1: card.type_line.split(" ").length, value2: otherCard?.type_line.split(" ").length },
    { label: "Keywords", value1: card.keywords?.length, value2: otherCard?.keywords?.length }
  ];
      ...
      <table className="mt-4 w-full text-sm">
        ...
        {comparisonData.map(({ label, value1, value2 }) => (
          <tr key={label}>
            ...
            <td className="px-2 py-1 text-right">{value1 && value2 && renderDifference(label, value1, value2)}</td>
          </tr>
        ))}
        </tbody>
      </table>
