...
export default function AdvancedSearch() {
  const [selectedColors] = useQueryState(
    "colors",
    parseAsArrayOf(parseAsString).withDefault([]),
  );
  const [selectedCardTypes] = useQueryState(
    "cardTypes",
    parseAsArrayOf(parseAsString).withDefault([]),
  );
  const [selectedRarity] = useQueryState(
    "rarity",
    parseAsString.withDefault(""),
  );
  const [power] = useQueryState(
    "power",
  ...
  const [legalities] = useQueryState(
    "legalities",
    parseAsArrayOf(parseAsString).withDefault([]),
  );
  ...
  return (
    <div className="mt-4">
      ...
      {isSuccess && query.length > 0 && (
        <>
          <h2 className="text-center text-3xl font-semibold">Search Results</h2>
          {data.pages.length > 0 ? (
