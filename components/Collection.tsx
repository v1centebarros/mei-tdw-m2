    ...
    <Drawer>
      ...
      <DrawerContent>
        <DrawerHeader className={"mx-auto justify-items-center"}>
          ...
          <DrawerDescription>You can check your favorite, want to buy, and owned cards here.</DrawerDescription>
        </DrawerHeader>
        <Tabs defaultValue="favorites" className="w-full justify-items-center">
          ...
          <TabsContent value="favorites">
            <div className={"flex flex-row gap-2"}>
              {favoriteCards.length > 0 ? (favoriteCards.map((card) => (<CardContextMenu card={card} key={card.id}>
                <Card card={card} key={card.id} />
              </CardContextMenu>))) : (<p>No favorite cards found</p>)}
            </div>
          </TabsContent>
          <TabsContent value="wantToBuy">
            <div className={"flex flex-row gap-2 "}>
              {wantToBuyCards.length > 0 ? (wantToBuyCards.map((card) => (<CardContextMenu card={card} key={card.id}>
                <Card card={card} key={card.id} />
              </CardContextMenu>))) : (<p>No cards in want to buy list</p>)}
            </div>
          </TabsContent>
          <TabsContent value="owned">
            <div className={"flex flex-row gap-2 overflow-x-auto"}>
              {ownedCards.length > 0 ? (ownedCards.map((card) => (<CardContextMenu card={card} key={card.id}>
                <Card card={card} key={card.id} />
              </CardContextMenu>))) : (<p>No owned cards found</p>)}
            </div>
          </TabsContent>
        </Tabs>
        <DrawerFooter>
