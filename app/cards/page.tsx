  ...
  return symbols.map((symbol, index) => {
    ...
    return (
      <Link
        ...
        href={
          foundSymbol.colors.length > 0
            ? `/search?colors=${foundSymbol.colors}`
            : "#"
        }
      >
        <Image
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <p>
                    ...
                    <Link
                      className={"underline-offset-2 hover:underline"}
                      href={`/search?rarity=${cardData.rarity}`}
                    >
                      {cardData.rarity}
                    </Link>
                  </p>
                  <p>
                    Power/Toughness:{" "}
                    <Link
                      href={`/search?power=${cardData.power},${cardData.power}`}
                      className={"underline-offset-2 hover:underline"}
                    >
                      {cardData.power}
                    </Link>
                    ...
                    {Object.entries(cardData.legalities).map(
                      ([format, legality]) => (
                        <Badge
                          key={format}
                          variant={
                            legality === "legal" ? "default" : "secondary"
                          }
                        >
                          <Link
                            key={format}
                            href={`/search?legalities=${format}`}
                            as={`/search?legalities=${format}`}
                          >
